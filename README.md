   ...
   - ## Limites do Projeto (Exclusões):
    ...
    - ### Diagramas: 
        ...
        - ## Diagrama de Fluxo:
            ```mermaid
                graph TD
                        A[Início: Acesso à Página] --> B(Navegar para /login);
                        B --> C{Preencher Credenciais?};
                        C --> |Não| B;
                        C --> |Sim| D(Chamar AuthService.login(credenciais));
                        D --> E{Login Sucesso?};
                        E --> |Não| F[Exibir Erro: Credenciais Inválidas] --> B;
                        E --> |Sim| G{Perfil do Usuário?};
                        G --> |Corretor| H[Redirecionar para /corretor/dashboard];
                        G --> |Cliente| I[Redirecionar para /cliente/interesses];

            ```
        - ## Diagrama de Fluxo Cliente:
            ```mermaid
                graph TD
                     A[Início: Página Pública] --> B(Buscar Imóveis);
                     B --> C{Imóvel de Interesse?};
                     C --> |Sim| D[Visualizar Detalhes do Imóvel];
                     C --> |Não| B;
                     D --> E{Está Logado?};
                     E --> |Não| F[Solicitar Login/Registro];
                     F --> G(Fazer Login ou Registrar);
                     E --> |Sim| H(Clicar em 'Tenho Interesse');
                     G --> H;
                     H --> I[Chamar InteressesService.marcarInteresse()];
                     I --> J[Imóvel Adicionado em 'Meus Interesses'];
                     J --> K[Acessar /cliente/interesses (Protegido por AuthGuard)];
            ```

            - ## Diagrama de Fluxo Corretor:
            ```mermaid
                graph TD
                        A[Início: Login como Corretor] --> B(Redirecionamento para /corretor/dashboard);
                        B --> C{Corretor Dashboard};
                        C --> D[Visualizar Meus Imóveis Cadastrados];
                        C --> E[Visualizar Clientes Interessados (Em cada Imóvel)];
                        D --> F{Ação de CRUD?};
                        F --> |Criar Novo| G[Navegar para Formulário de Criação];
                        F --> |Editar Existente| H[Navegar para Formulário de Edição (ID)];
                        F --> |Excluir| I[Confirmação e Exclusão (Chamar ImoveisService.delete())];
                        G --> J(Preencher Dados do Imóvel);
                        H --> J;
                        J --> K[Salvar (Chamar ImoveisService.post/put())];
                        K --> D;
            ```